...
def parse_args():
    ...
    parser.add_argument('--draw', dest='draw',
                        ...
                        default=0, type=int)
    parser.add_argument('--car-only', dest='car_only',
                        help='only use car class',
                        default=0, type=int)
    parser.add_argument('--dontcareothers', dest='dontcareothers',
                        help='ignore other categories, add them to dontcare regions',
                        default=0, type=int)
...
LABEL_DIR = ['training/label_2', 'training/label_2_car_new']
# It is better to use absolute path for TRAIN_FILE_DIR and TEST_FILE_DIR
TRAIN_FILE_DIR = ['../data/ImageSets/train.txt', '../data/ImageSets_car_new/train.txt']
TEST_FILE_DIR = ['../data/ImageSets/val.txt', '../data/ImageSets_car_new/test.txt']
...
if __name__ == '__main__':
    ...
    _car_only = args.car_only
    _dest_label_dir, _dest_img_dir, _dest_set_dir = build_voc_dirs(_outdir)  # Annotations, JPEGImages, ImageSets/Main
    _doncareothers = bool(args.dontcareothers)

    train_files = []
    test_files = []
    _labeldir = os.path.join(_kittidir, LABEL_DIR[_car_only])

    files = glob.glob(os.path.join(_labeldir, '*.txt'))
    files.sort()

    train_file_dir = TRAIN_FILE_DIR[_car_only]
    test_files_dir = TEST_FILE_DIR[_car_only]

    with open(train_file_dir, 'rb') as f:
        lines = f.readlines()
    train_files = [os.path.join(_labeldir, '{}.txt'.format(line.strip())) for line in lines]
    train_files.sort()
    with open(test_files_dir, 'rb') as f:
        lines = f.readlines()
    test_files = [os.path.join(_labeldir, '{}.txt'.format(line.strip())) for line in lines]
    test_files.sort()
    ...
    for dset in ['train', 'test']:
        ...
        if _car_only:
            ...
            class_sets = ('pedestrian', 'cyclist', 'dontcare')
        ...
        for file in tqdm(files):
            ...
            doc, objs = generate_xml(stem, lines, img_size, class_sets=class_sets, doncareothers=_doncareothers)
            if _draw:
            ...
            for obj in objs:
                cls = obj['class']
                all_classes[cls] = 0 \
                    if not cls in all_classes.keys() else all_classes[cls] + 1
