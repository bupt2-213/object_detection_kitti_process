# convert kitti data to voc format, include classes: car, pedestrian, cyclist
...
if __name__ == '__main__':
    ...
    files.sort()
    random.seed(4)
    train_files = random.sample(files, int(len(files)*0.7))  # 70% as train
    train_files.sort()
    test_files = list(set(files) - set(train_files))
    test_files.sort()

    # # for kitti only provides training labels, so divide training into 30% test and 70% train
